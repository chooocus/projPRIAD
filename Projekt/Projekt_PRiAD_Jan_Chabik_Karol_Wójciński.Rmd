...
# Podstawy reprezentacji i analizy danych – projekt
 ...
 #### Rozpoznawanie ocen końcowych na podstawie ankiet portugaliskich uczniów matematyki
 Projekt realizowany w ramach laboratorium Podstawy reprezentacji i analizy danych. Wykonali:
 - Jan Chabik
 - Karol Wójciński
...
Dane, które wykorzystujemy zostały zebrane za pomocą przeprowadzonych ankiet rozwiązanych przez uczniów matematyki i języka portugalskiego w szkole średniej. Dane zawierają wiele interesujących socjalnych, płciowych i naukowych informacji na temat uczniów. Końcowe oceny zostały już przewidziane. Naszym zadaniem jest, wykorzystując wiedzę zdobytą na laboratoriach, by za pomocą tych danych przeprowadzić badania eksploracyjne. Korzystać będziemy z danych uzyskanych dzięki uczniom matematyki.
...
30.	absences - liczba nieobecności w szkole (numerycznie: od 0 do 93) 
...
```{python}
plt.figure(figsize=(29, 29), dpi = 150)
...
sns.heatmap(df.corr().iloc[-3:, :-3], annot = df.corr().iloc[-3:, :-3])
```

#### Posortowana tabela z wpływem atrybutów na oceny

```{python}
posortowanaKorelacja = df.corr().iloc[-3:, :-3].mean().abs().sort_values(ascending=False)
...
```{python}
parametryZnaczace = list(list(posortowanaKorelacja.index[:6]))

print('Parametry znaczące:')
for i, v in enumerate(parametryZnaczace):
    print (v)
```
...
sns.heatmap(df[parametryZnaczace].corr(), annot=df[parametryZnaczace].corr())
```
...
## 15. Wybór optymalnego klasyfikatora


Skorzystaliśmy z funkcji pomocnicznych, które można znaleźć w materiałach na ISODzie w pliku $PRiAD 5 - Uczenie$ $nadzorowane.ipynb$
...
## 16. Ocena klasyfikacji


Testowaliśmy różne klasyfikatory, aby zobaczyć, który najlepiej spełnił swoje zadanie. Na podstawie macierzy błędów uznaliśmy, iż klasyfikator najbliższych prototypów najbardziej pasuje do naszego zbioru danych.


## 17. Wnioski


Zbiór danych student-mat jest obszernym zbiorem danych, o wielu atrybutach. Jego analiza przy pomocy narzędzi analizy danych pozwala na zaobserwowanie ciekawych korelacji (np. wykształcenie matki jako trzeci najważniejszy czynnik).
Mapy ciepła pozwalają sprawnie oddzielić atrybuty znaczące od reszty i na ich podstawie przeprowadzić dalszą analizę.
Dobór klasyfikatora przeprowadziliśmy metodą prób i błędów, decydując który nam najbardziej odpowiada na podstawie macierzy pomyłek. Dla takiego zbioru można by również zastosować regresję, niestety nie znaleźliśmy potrzebnych materiałów w zasobach na $Isodzie$.
